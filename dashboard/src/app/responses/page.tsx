...
function text(value: string | string[] | undefined, limit = 80) {
...
export default async function ResponsesPage({
  ...
  const scopedParticipants = applyParticipantFilters(
    data.participants,
    filters
  ).filter(
    (item) =>
      !participant ||
      item.participantCode.toLocaleLowerCase().includes(participant)
  );
  const rows = buildPublicResponseRows(scopedParticipants, config).filter(
    (row) =>
      (!question || row.questionId === question) &&
      (!construct || row.construct === construct) &&
      (!responseType || row.responseType === responseType) &&
      (presence !== "answered" || row.answerState === "ANSWERED") &&
      (presence !== "missing" || row.answerState !== "ANSWERED") &&
      (!from || (row.createdAt ?? "") >= `${from}T00:00:00`) &&
      (!to || (row.createdAt ?? "") <= `${to}T23:59:59`)
  );
  ...
  return (
    <main className="page">
      <header className="page-header">
        <div>
          <p className="eyebrow">Long-format explorer</p>
          <h1>All Responses</h1>
          <p className="lede">
            Researcher-friendly response records with server-side privacy
            redaction.
          </p>
        </div>
      ...
      <form className="filter-bar" method="get">
        ...
        <label>
          ...
          <input
            name="participant"
            defaultValue={text(params.participant)}
            placeholder="P007"
          />
        </label>
        <label>
          Question
          <input name="question" defaultValue={question} placeholder="q7" />
        </label>
        <label>
          Construct
          <input name="construct" defaultValue={construct} />
        </label>
        <label>
          Response type
          <input name="responseType" defaultValue={responseType} />
        </label>
        <label>
          Answer presence
          <select name="presence" defaultValue={presence}>
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={`${row.participantCode}-${row.questionId}`}>
                <td>
                  <Link href={`/participants/${row.participantCode}`}>
                    {row.participantCode}
                  </Link>
                </td>
                <td>
                  <Link href={`/questions/${row.questionId}`}>
                    {row.questionId}
                  </Link>
                  <br />
                  <span className="muted">{row.question}</span>
                </td>
                <td>
                  {humanize(row.construct)}
                  <br />
                ...
                <td>
                  ...
                  {row.rawValue !== undefined && (
                    <details>
                      ...
                      <pre>{JSON.stringify(row.rawValue, null, 2)}</pre>
                    </details>
                  )}
                </td>
