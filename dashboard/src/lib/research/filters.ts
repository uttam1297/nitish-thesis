...
export function applyParticipantFilters(participants: readonly ParticipantViewModel[], filters: DashboardFilters): ParticipantViewModel[] {
  ...
  return participants.filter((participant) =>
    ...
    (!filters.closeness || participant.discoveryCloseness.startsWith(filters.closeness)) &&
    (filters.completeness === "all" || (filters.completeness === "complete" ? participant.coverage === 1 : participant.coverage < 1)),
  );
}
