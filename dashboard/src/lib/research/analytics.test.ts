...
function participant(overrides: Partial<ParticipantViewModel> = {}): ParticipantViewModel {
  return {
    participantCode: "P002", roles: ["product"], industry: "Retail / E-commerce", experience: "3-6 years",
    discoveryCloseness: "4 of 5", status: "completed", studyStage: "main", responseMode: "asynchronous_form",
    questionnaireVersion: "1.3.0", startedAt: "2026-09-15T10:00:00Z", lastActivityAt: "2026-09-15T11:00:00Z",
    completedAt: "2026-09-15T11:00:00Z", withdrawnAt: null, answeredCount: 16, expectedCount: 16,
    missingQuestionIds: [], hiddenQuestionIds: [], coverage: 1, responses: [], consent: null, ...overrides,
  };
}
...
describe("deterministic dataset analytics", () => {
  it("uses expected answers as the dataset coverage denominator", () => {
    const metrics = calculateDatasetMetrics([
      participant(),
      participant({ participantCode: "P003", answeredCount: 8, expectedCount: 16, status: "in_progress" }),
    ]);
  ...
  it("excludes withdrawn sessions from response metrics", () => {
    const metrics = calculateDatasetMetrics([
      participant(),
      participant({ participantCode: "P004", status: "withdrawn", answeredCount: 16, expectedCount: 16 }),
    ]);
