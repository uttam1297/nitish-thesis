...
export function validateResponseValue(
  ...
  if (!isRecord(value) || typeof value.kind !== "string") {
    return { valid: false, diagnostic: "Expected a response object with a kind." };
  }
  ...
  if (question.responseType === "multi_select") {
    if (
      ...
      return { valid: false, diagnostic: "Expected one or more string choices." };
    }
    return {
      valid: true,
      answer: {
        kind: "choices",
        values: value.values,
        ...(optionalOtherText(value) ? { otherText: optionalOtherText(value) } : {}),
      },
    };
  }
  ...
  if (question.responseType === "likert_scale") {
    if (
      ...
      return { valid: false, diagnostic: "Expected an integer inside the configured scale." };
    }
  ...
  if (
    ...
    value.text.trim().length < question.validation.minimumNonWhitespaceCharacters
  ) {
...
function optionLabel(question: QuestionMetadata, value: string, otherText?: string): string {
  if (value === "__other__") return otherText ? `Other — ${otherText}` : "Other";
  if (question.responseType !== "multi_select" && question.responseType !== "single_select") {
    return value;
  }
  return question.options.find((option) => option.value === value)?.label ?? `Unknown option (${value})`;
}

export function answerToReadable(answer: ValidAnswer, question: QuestionMetadata): string {
  if (answer.kind === "choice") return optionLabel(question, answer.value, answer.otherText);
  if (answer.kind === "choices") {
    return answer.values.map((value) => optionLabel(question, value, answer.otherText)).join(", ");
  }
