...
import { SESSION_STATUSES, RESPONSE_MODES } from "@/lib/google-sheets/records";
...
export default async function AdminOverviewPage({
  ...
  const filteredSessions = sessions.filter(
    ({ record }) =>
      ...
      (!modeFilter || record.responseMode === modeFilter)
  );
            ...
            {filteredSessions.map(({ record }) => {
              ...
              return (
                <tr key={record.sessionId} className="border-b last:border-0">
                  ...
                  <td className="px-4 py-3">
                    {record.responseMode === "live_interview"
                      ? "Live interview"
                      : "Async form"}
                  </td>
                  <td className="px-4 py-3">{record.studyStage}</td>
