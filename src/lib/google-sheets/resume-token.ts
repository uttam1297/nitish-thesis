 ...
 * credential, and must never be treated as one — see
 * `records.ts`/`participant-repository.ts`.
 */
...
/** Only this hash — never the raw token — is stored in the Sessions sheet. */
export function hashResumeToken(token: string): string {
  return createHash("sha256").update(token).digest("hex");
}

/** Constant-time comparison so a slow-scan lookup can't leak timing info. */
