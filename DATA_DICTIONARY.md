...
| `voice_input_consent`           | boolean     | Agreement to speak answers into the form. `true` for sessions on a questionnaire that offers voice input (1.5.0 does). Transcription runs in the participant's browser, so no audio is ever received or stored. **Not** the same as `recording_consent`. |
